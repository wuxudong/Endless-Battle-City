...
public class Tank extends Obstacle implements FrameAware {
    ...
    public Tank(MyPoint position, Ally ally, TankType tankType, boolean carryFood, Player player) {
                ...
                case SOUTH: {
                    ...
                    MyPoint nextReachablePosition =
                            reachablePosition(scene,
                                    ...
                                    direction);
                    position = new MyPoint(nextReachablePosition.x, nextReachablePosition.y - getHeight());
                    break;
                }
                case WEST: {
                ...
                case EAST: {
                    ...
                    MyPoint nextReachablePosition =
                            reachablePosition(scene,
                                    ...
                                    direction);
                    position = new MyPoint(nextReachablePosition.x - getWidth(), nextReachablePosition.y);
                    break;
                }
    ...
    private Bullet fire(final Scene scene) {
        if (!fire) {
            ...
            if (coolingFrame == 0) {
                ...
                return new Bullet(power, bulletSpeed, new MyPoint(x, y), direction, this);
            } else {
                return null;
            }
        }
    ...
    private MyPoint reachablePosition(Scene scene, Rect rect, Direction direction) {
        ...
        switch (direction) {
            case NORTH:
                return new MyPoint(rect.left, reachableNorthY);
            case SOUTH:
                return new MyPoint(rect.left, reachableSouthY);
            case WEST:
                return new MyPoint(reachableWestX, rect.top);
            default:
                return new MyPoint(reachableEastX, rect.top);
        }
    }
